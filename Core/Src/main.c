...
//	uint32_t wait = 500;
  ...
  while (1)
  {
	  ...
	  int is_pressed = !(HAL_GPIO_ReadPin(GPIOC, GPIO_PIN_15));

	  if (is_press_watching && !is_pressed){
		  press_duration = HAL_GetTick() - start_time_pressing;
		  is_press_watching = 0;
	  ...
	  if (!is_press_watching && is_pressed){
		  start_time_pressing = HAL_GetTick();
		  is_press_watching = 1;
		  // continue;
	  }
